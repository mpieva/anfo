 ...
 *
 * Our main configuration is of type metaindex::MetaIndex, pointers to
...
// \param genome_name name to look for
// \return A reference to the genome config.
metaindex::Genome& find_or_create_genome( metaindex::MetaIndex& mi, std::string genome_name ) ;
...
const metaindex::CompactIndex& find_compact_index(
		const metaindex::MetaIndex& mi, const std::string genome_name, unsigned wordsize = 0 ) ; 
